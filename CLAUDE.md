...
Beyond those reference scenarios, `order-service` also exposes `GET /orders/{sagaId}`: while the order is still `PENDING` it answers from a live Temporal `@QueryMethod` call (`OrderSagaWorkflow.getProgress()`) instead of a `saga_step`-style read model table; once terminal it answers from the durable `customer_order` row alone. See [`docs/order-adjustment-and-status-query-plan.md`](./docs/order-adjustment-and-status-query-plan.md) for the design rationale. Covered by `OrderQueryHandlerTest`/`OrderControllerTest`/`OrderSagaWorkflowImplTest` and `platform-test/scripts/e2e-order-status-query.sh`.
